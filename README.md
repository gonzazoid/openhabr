...
- start_session - на основании полученных кук запускает сессию - тупо подтягивает запись из твблицы users и кладет ее в job.request.user
- worker - а это как раз наз обработчик, который мы сейчас начнем реализовывать
